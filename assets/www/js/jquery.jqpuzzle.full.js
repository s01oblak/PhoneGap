 ...
 */
$.fn.jqPuzzle = function(settings, texts, callback) {
	...
	var defaults = {
		...
		style: {
			...
			overlap: false,			// if true, adjacent piece borders will overlap
	...
	return this.filter('img').each(function(){
		...
		*/
			...
			if ((settings.vidni_width == -1) || (settings.vidni_height == -1)){
				...
				settings.window_width = $srcImg.width();
				settings.window_height = $srcImg.height();
			...
			var $background = $('<div/>')
				.css({
					...
					backgroundImage: 'url(' + imgSrc + ')',
					backgroundSize: settings.width_celotne_slike_po_cropanju + 'px ' + settings.height_celotne_slike_po_cropanju + 'px',
					backgroundPosition: '-' + settings.odmik_levo + 'px -' + settings.odmik_zgoraj + 'px',
					borderWidth: 0,
					margin: 0,
					padding: 0,
					position: 'absolute',
